...
export default function MarionetteAnimation({ onAnimationEnd }: MarionetteAnimationProps) {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => onAnimationEnd(), 9500),
    ];
  ...
  return (
    <div className="marionette-container">
      <svg
        className="marionette"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 306.4384 100.049"
      >
      ...
      <div className="story">
        <div className="story__content story__content--top">
          <h1 className={`blurb blurb--one ${phase >= 1 ? "visible" : ""}`}>
            Avui no surto mama,
          </h1>
          <h1 className={`blurb blurb--three ${phase >= 3 ? "visible" : ""}`}>
            Beure? Noooo...
          </h1>
        </div>
        <div className="story__content story__content--bottom">
          <h1 className={`blurb blurb--two ${phase >= 2 ? "visible" : ""}`}>
            només vaig a el "huerto".
          </h1>
          <h1 className={`blurb blurb--four ${phase >= 4 ? "visible" : ""}`}>
            Avui nit tranquileta, zero alcohol!
          </h1>
        </div>
      </div>
    </div>
